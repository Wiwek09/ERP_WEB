...
import AdditionalPurchaseDtl from "./components/AdditionalPurchaseDtl";
...
const initialSelectedProducts: ISelectedProduct = {
  PurchaseId: 0,
...
const getLocalItem = (key: string, id: string) => {
  ...
  return parsedData.filter((item: ISelectedProduct) => item.InventoryItemId);
};
...
const ManagePurchase = () => {
  ...
  const [additionalPurchaseDtl, setAdditionalPurchaseDtl] =
    useState<IAdditionalPurchaseDtl>({
      ...purchase,
      ...otherpurchase,
      invoiceNo: "",
      voucherDate: getNepaliDate(),
  ...
  const [selectedAccountData, setSelectedAccountData] = useState<IVoucher[]>([
    { ...initailAccountTransactionValue, IsExciseDuty: true },
    { ...initailAccountTransactionValue, IsDiscount: true },
    { ...initailAccountTransactionValue, IsTaxable: true },
    { ...initailAccountTransactionValue, IsVAT: true },
    { ...initailAccountTransactionValue, IsNonTaxable: true },
  ]);
  ...
  useEffect(() => {
    ...
    selectedProductData.forEach((element) => {
      ...
      if (additionalPurchaseDtl.voucherType === "Purchase Non Vat") {
        ...
        if (element.TaxRate > 0) {
          taxableTotal +=
            element.Quantity *
            (element.PurchaseRate + element.ExciseDuty - element.Discount);
        } else {
          nontaxableTotal += element.Quantity * element.PurchaseRate;
          +element.ExciseDuty - element.Discount;
        }
      }
    });
    ...
    setTotalTaxable(
      additionalPurchaseDtl.voucherType === "Purchase Non Vat"
        ? 0
        : isFirstLoad && Id.taxable.debit > 0
        ? Id.taxable.debit
        : taxableTotal
    );
    setTotalNonTaxable(
      isFirstLoad && Id.nonTaxable.debit > 0
        ? Id.nonTaxable.debit
        : nontaxableTotal
    );
    setTotalDiscount(
      isFirstLoad && Id.discount.debit > 0 ? Id.discount.debit : discounttotal
    );
    selectedAccountData?.forEach((data) => {
      if (data.IsDiscount) {
        if (data.Debit === 0) {
  ...
  const fetchPurchaseData = async () => {
    try {
      ...
      console.log(response,"Response-Purchase")
      const getExciseDutyAmount = (
        itemRate: number,
        discount: number,
        exciseDuty: number
      ) => {
        ...
        const exciseDutyAmount =
          (exciseDuty / 100) * itemRate;
        return exciseDutyAmount;
      };
      const GetAfterExciseDuty = (
        itemRate: number,
        exciseDutyAmount: number,
      ) => {
        const AfterExciseDuty = itemRate + exciseDutyAmount;
        return AfterExciseDuty;
      };
      const GetBeforePriceVAT = (
        itemRate: number,
        DiscountAmount: number,
      ) => {
        const BeforePriceVAT = itemRate - DiscountAmount;
        return BeforePriceVAT;
      };
      const GetTotalPurchaseValue = (AfterVATRate: number, Qty: number) => {
        const TotalPurchaseValue = AfterVATRate * Qty;
        return TotalPurchaseValue;
      };
      const getAfterVatAmount = (
        itemRate: number,
        exciseDutyAmount: number,
        discount : number,
        taxRate: number
      ) => {
        const afterVatAmount = ((itemRate + exciseDutyAmount) * taxRate/100) + itemRate + exciseDutyAmount - discount;
      ...
      const productData = await Promise.all(
        response.PurchaseDetails.map((data) => {
          ...
          return {
            ...
            AfterExciseDuty: GetAfterExciseDuty(data.PurchaseRate, exciseDutyAmount), 
            BeforePriceVAT : GetBeforePriceVAT(GetAfterExciseDuty(data.PurchaseRate, exciseDutyAmount), data.Discount),
            AfterVatAmount: getAfterVatAmount(data.PurchaseRate, exciseDutyAmount, data.Discount,data.TaxRate),
            TotalPurchaseValue : GetTotalPurchaseValue(getAfterVatAmount(data.PurchaseRate, exciseDutyAmount, data.Discount,data.TaxRate), data.Quantity)
          };
        })
      );
      if (id !== "add") {
        ...
        const localData =
          data.filter((item: ISelectedProduct) => {
            return !productData.find(
              (item2: ISelectedProduct) => item2.PurchaseId === item.PurchaseId
            );
          }) || [];
  ...
  useEffect(() => {
    const fetchData = async () => {
      setAllData();
      if (loginedUserRole.includes("PurAdd") && id === "add") {
        setIsPurchaseDataLoaded(true);
      } else if (loginedUserRole.includes("PurEdit") && id !== "add") {
        setFirstLoad(true);
  ...
  const updateSelectedProduct = (
    ...
    if (name === "InventoryItemId") {
        ...
        selectedProductData.map((data, i) => {
          if (i === index) {
            return {
              ...data,
              Quantity: value,
              PurchaseAmount: (data.PurchaseRate + data.ExciseDuty - data.Discount ) * value,
              TotalPurchaseValue : (data.PurchaseRate + data.ExciseDuty - data.Discount + (data.TaxRate / 100) * (data.PurchaseRate + data.ExciseDuty - data.Discount)) * value
            };
          }
          return data;
        })
      );
    } else if (name === "PurchaseRate") {
      setSelectedProductData(
        selectedProductData.map((data, i) => {
          if (i === index) {
            let afterVatAmount = 0;

            let ExciseDuty = 0;
            let AfterExciseDuty = 0;

            if (data.ExciseDutyRate > 0) {
              ExciseDuty =
                (data.ExciseDutyRate / 100) * (value - data.Discount);

              AfterExciseDuty = value + ExciseDuty - data.Discount;
            }
            afterVatAmount =
              (data.TaxRate / 100) * (value + ExciseDuty - data.Discount) +
              (value + ExciseDuty - data.Discount);
            return {
              ...data,
              PurchaseRate: value,
              PurchaseAmount:  data.Quantity *  (value + ExciseDuty - data.Discount),
              TotalPurchaseValue : afterVatAmount * data.Quantity,
              AfterVatAmount: afterVatAmount,
              BeforePriceVAT : value,
              ExciseDuty,
              AfterExciseDuty,
            };
          }
    ...
    } else if (name === "ExciseDutyRate") {
      setSelectedProductData(
        selectedProductData.map((data, i) => {
          if (i === index) {
            const ExciseDutyRate = value;

            let ExciseDuty =
              (ExciseDutyRate / 100) *  Number(data.PurchaseRate);
            ...
            afterVatAmount =
            (data.TaxRate / 100) *
              (data.PurchaseRate + ExciseDuty - data.Discount) +
            (data.PurchaseRate + ExciseDuty - data.Discount);
          AfterExciseDuty =
            Number(data.PurchaseRate) + ExciseDuty - data.Discount;

            return {
              ...data,
              ExciseDutyRate: value,
              ExciseDuty: ExciseDuty,
              AfterExciseDuty: AfterExciseDuty,
              PurchaseAmount: data.Quantity *  AfterExciseDuty,
              TotalPurchaseValue : data.Quantity * afterVatAmount,
              BeforePriceVAT : data.PurchaseRate + ExciseDuty,
              AfterVatAmount: afterVatAmount,
            };
          }
          return data;
      ...
      setSelectedProductData(
        selectedProductData.map((data, i) => {
          if (i === index) {
            let TaxRate = value;
            const taxAmount = (data.PurchaseRate * TaxRate) / 100;

            let afterVatAmount = 0;
            afterVatAmount = taxAmount + data.BeforePriceVAT;
      ...
      setSelectedProductData(
        selectedProductData.map((data, i) => {
          if (i === index) {
            ...
            let ExciseDuty =
              (data.ExciseDutyRate / 100) *  Number(data.PurchaseRate);

            if (data.TaxRate > 0) {
              afterVatAmount =
                (data.TaxRate / 100) *
                  (data.PurchaseRate + ExciseDuty  - value) +
                (data.PurchaseRate  + ExciseDuty - value);
            }
            ...
            return {
              ...data,
              Discount: value,
              PurchaseAmount: (data.PurchaseRate + ExciseDuty - value) * data.Quantity,
              TotalPurchaseValue : data.Quantity * afterVatAmount,
              BeforePriceVAT : data.PurchaseRate + ExciseDuty - value,  
              AfterVatAmount: afterVatAmount,
            };
          }
          return data;
  ...
  const deleteSelectedProduct = async (index: number) => {
    ...
    if (Id && id !== "add") {
      const savedItems = selectedProductData.filter(
        (item: ISelectedProduct) => item.PurchaseId
      );
  ...
  const addPurchaseData = async () => {
    ...
    if (validInvoice) {
      if (validDate) {
        ...
        // console.log("My console Log taxable / non taxable ", selectedVoucherData);
  ...
  const handleUpdatePurchaseDataOnDelete = async (index?: number) => {
    ...
    let accountTransactionValues: IVoucher[] = selectedVoucherData.map(
      (data) => {
        return {
          ...data,
          Name: additionalPurchaseDtl.voucherType,
    ...
    for (const [index, data] of selectedAccountData.entries()) {
      const containValue = accountTransactionValues.some(
      ...
      if (!containValue) {
        ...
        accountTransactionValues.push(totalDebitVoucher);
      }
      ...
      if (data.entityLists === "Cr") {
        ...
        accountTransactionValues.push(creditVoucher);
      }
    }
    ...
    const data: IPurchase | null = purchaseData
      ? {
          ...
          AccountTransactionValues:
            ...
            accountTransactionValues[0].AccountId === 0
              ? []
              : accountTransactionValues,
          Date: additionalPurchaseDtl.voucherDate,
          PurchaseDetails: index
            ? products?.filter((item: ISelectedProduct, i) => i !== index)
            : products,
          Amount: grandTotal+totalVoucher,
          DebitAmount: grandTotal+totalVoucher,
          drTotal: grandTotal+totalVoucher,
  ...
  const deletePurchaseData = async () => {
    try {
      ...
      successMessage("Purchase is successfully deleted.");
      resetAll();
      history.push("/purchase");
    } catch {
  ...
  return (
    <>
      <Box>
        <FormHeader
          headerName={id === "add" ? "Add Purchase" : "Edit Purchase"}
        />
        {isPurchaseDataLoaded && areAllDataLoaded ? (
          <Paper sx={{ paddingX: 3, paddingY: 4 }}>
            ...
            <Products
              ...
              // updateMoreDetails={updateSelectedVoucherData}
