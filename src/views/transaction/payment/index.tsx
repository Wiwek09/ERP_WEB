...
const Payment = () => {
  ...
  const getData = async () => {
    ...
    if(selectbranchId.branch === 0){
      ...
      const response = await getAllPaymentsByBranchFilter(
        dateChoose.StartDate,
        dateChoose.EndDate,
  ...
  const editDeletePayment = (id: number) => {
  ...
  const setAction = (actionType: string, id: number) => {
    switch (actionType) {
      case "edit":
        if(loginedUserRole.includes("PaymentEdit")){
          editDeletePayment(id);
          break;
        }else{
      ...
      case "delete":
        if(loginedUserRole.includes("PaymentDelete")){
          editDeletePayment(id);
          break;
        }else{
          errorMessage("Sorry, permission denied.");
      ...
      case "view":
        if(loginedUserRole.includes("PaymentView")){
          viewPayment(id);
          break;
        }else{
          errorMessage("Sorry, permission denied.");
...
//for branch
  ...
  return (
    <>
      {
        loginedUserRole.includes("PaymentAdd") ?
        <GridDateHeader
          ...
          fileName={`payment-voucher-report-${dateChoose.StartDate}-${dateChoose.EndDate}`}
        /> :
        <GridDateHeader
          headerName={"Payment Voucher"}
          dateChoose={dateChoose}
      ...
      {isLoading ? (
            ...
            <Table stickyHeader aria-label="sticky table" sx={{ minWidth: 650 }}>
              ...
              />
              <TableBody>
                {stableSort(paymentData, getComparator(order, orderBy)).map((data:IAllVoucher, index:number) => {
                    return (
                      <>
